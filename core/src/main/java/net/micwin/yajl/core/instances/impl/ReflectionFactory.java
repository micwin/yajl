 ...
 * <ul>
 * <li><code>DEFAULT_CONSTRUCTOR</code>: create instances by simply calling the
 * default constructor</li>
 * <li><code>CONFIG_CONSTRUCTOR</code>: create instances by calling the
 * constructor taking an instance of <code>IConfiguration</code> that is created
 * using the config's sub group <code>config</code>.</li>
...
public class ReflectionFactory<T> implements IFactory {
	...
	/**
	 * key <code>class</code> should contain the fully qualified name to the
	 * class to be invoked.
	 */
	...
	/**
	 * If strategy <code>CONFIG_CONSTRUCTOR</code> is used, then this key is
	 * used to create the instance of IConfiguration that is to passed to the
	 * constructor.
	 */
	public static final String KEY_INVOCATION_CONFIG = "config";

	/**
	 * If strategy <code>STATIC_FIELD</code> is used, then this key holds the
	 * name of the appropriate static field. or <code>STATIC_FIELD</code> is
	...
	public ReflectionFactory(IConfiguration configuration)
		...
		case CONFIG_CONSTRUCTOR:
			try {
				invoker = new ConfigurationInvoker(clazz,
						configuration.sub(KEY_INVOCATION_CONFIG));
